...
public class InstanceGraph extends ExtendedTopologyGraph<InstanceVertex> {
  ...
  public static final String GRAPHVIZ_FILENAME = "instance.gv";

  /**
   * The process graph from which this instance graph is created.
   */
  private ProcessGraph rpg;

  /**
   * A collection of Instances and its constituent processes
   */
  private Map<Instance, Set<Process>> instances;

  /**
   * Reverse mapping from process to instance
   */
  private Map<Process, Instance> process2instance;
  ...
  public boolean customize(Flow flow) {
    return this.customize(flow, new InstanceVertex(null, VertexType.SOURCE),
  ...
  protected void constructEndpointEdges() {
    ProcessVertex rpg_source = this.rpg.getFlowSourceVertex();
    for (DirectedEdge<ProcessVertex> src_edge :
      ...
      this.addEdge(this.getFlowSourceVertex(), vertex, 1.0,
  ...
  protected void filterEdgesAndVertices() {
    ...
    // things that remove edges would already have been taken care of in
    // ProcessGraph
  }
